...
class CarvanaClassifier:
    def __init__(self, net, max_epochs, device):
        """
    ...
    def _criterion(self, logits, labels):
        return losses_utils.BinaryCrossEntropyLoss2d().forward(
            logits, labels) + losses_utils.SoftDiceLoss().forward(logits, labels)
        ...
        with tqdm(total=it_count,
            ...
            for ind, (inputs, target) in enumerate(train_loader):
                ...
                pbar.set_postfix(
                    OrderedDict(
                        loss='{0:1.5f}'.format(
                            loss.item()), dice_coeff='{0:1.5f}'.format(
                            acc.item())))
                pbar.update(1)
    ...
    def _run_epoch(self, train_loader, valid_loader,
        ...
        train_loss, train_dice_coeff = self._train_epoch(
            train_loader, optimizer, threshold)
        ...
        # If there are callback call their __call__ method and pass in some
        # arguments
        if callbacks:
            for cb in callbacks:
                cb(step_name="epoch",
    ...
    def predict(self, test_loader, callbacks=None):
        ...
        with tqdm(total=it_count, desc="Classifying") as pbar:
            for ind, (images, files_name) in enumerate(test_loader):
                ...
                # If there are callback call their __call__ method and pass in
                # some arguments
                if callbacks:
                    for cb in callbacks:
